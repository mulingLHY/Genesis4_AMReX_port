...
bool Track::init(int inrank, int insize, map<string,string> *arg, Beam *beam, vector<Field *> *field,
                 Setup *setup, Lattice *lat, AlterLattice *alt, Time *time, FilterDiagnostics &filter)
{
  rank=inrank;
  size=insize;
  ...
  if (!use_cuda) {
    fftsolver=false; // CPU FFT solver needs FFTW; CUDA FFT solver uses cuFFT.
  }
#else
  ...
  lat->generateLattice(setup, alt, und);
  /* !changes to 'lat' after this function call may have no effect, as this function also stores the generated lattice into the provided Undulator instance! */
  if(dumpFieldUE) und->markUndExits();
  und->updateMarker(dumpFieldStep,dumpBeamStep,sort_step,zstop);
  // update first the marker to calculate correctly the stopping length (otherwise it could be bypassed by the lattice file)
  ...
  if (ssrun){
    // disable global output when there is only one slice calculated
  ...
  for (int i=0; i<field->size();i++){
    field->at(i)->setOutput( setup->outputFFT(), setup->outputSpatial(), setup->outputIntensity(), setup->outputFieldDump());
  }
  ...
  Gencore core;
  if(!core.run(beam,field,setup,und,isTime,isScan, periodic, use_cuda, filter)) {
