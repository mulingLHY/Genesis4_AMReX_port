...
struct BeamSolverCUDA::Buffers {
  amrex::Gpu::HostVector<double> h_particle_ez;
  amrex::Gpu::DeviceVector<double> d_particle_ez;
  amrex::Gpu::HostVector<double> h_slice_eloss;
  amrex::Gpu::DeviceVector<double> d_slice_eloss;

  amrex::Gpu::HostVector<int> h_first;
  amrex::Gpu::HostVector<int> h_ngrid;
  amrex::Gpu::HostVector<int> h_nslice;
  amrex::Gpu::HostVector<double> h_gridmax;
  amrex::Gpu::HostVector<double> h_dgrid;
  amrex::Gpu::HostVector<double> h_rtmp;
  amrex::Gpu::HostVector<double> h_rharm;
  amrex::Gpu::HostVector<const double*> h_field_re;
  amrex::Gpu::HostVector<const double*> h_field_im;

  amrex::Gpu::DeviceVector<int> d_first;
  amrex::Gpu::DeviceVector<int> d_ngrid;
  amrex::Gpu::DeviceVector<int> d_nslice;
  amrex::Gpu::DeviceVector<double> d_gridmax;
  amrex::Gpu::DeviceVector<double> d_dgrid;
  amrex::Gpu::DeviceVector<double> d_rtmp;
  amrex::Gpu::DeviceVector<double> d_rharm;
  amrex::Gpu::DeviceVector<const double*> d_field_re;
  amrex::Gpu::DeviceVector<const double*> d_field_im;
};
...
void BeamSolverCUDA::advance(double delz, Beam *beam, vector<Field *> *field, Undulator *und) {
  if (beam == nullptr || field == nullptr || und == nullptr) {
    amrex::Abort("BeamSolverCUDA::advance received a null beam/field/undulator pointer");
  }
  ...
  for (int i = 0; i < static_cast<int>(field->size()); ++i) {
    ...
    if (fsoa == nullptr || !fsoa->initialized || fld->ngrid <= 1) {
      amrex::Abort("BeamSolverCUDA::advance requires initialized FieldSoA");
    }
  ...
  if (nfield > kMaxBeamSolverFields) {
    amrex::Abort("BeamSolverCUDA::advance exceeds kMaxBeamSolverFields");
  }
  ...
  if (use_short_range) {
    ...
    if (particle_offset != total_particles) {
      amrex::Abort("BeamSolverCUDA::advance requires CPU beam slice sizes to match BeamSoA layout");
    }

    amrex::Gpu::copy(amrex::Gpu::hostToDevice,
                     work.h_particle_ez.begin(),
  ...
  if (nfield > 1) {
    ...
    amrex::Gpu::copy(amrex::Gpu::hostToDevice,
                     work.h_first.begin(), work.h_first.begin() + nfield, work.d_first.begin());
    amrex::Gpu::copy(amrex::Gpu::hostToDevice,
                     work.h_ngrid.begin(), work.h_ngrid.begin() + nfield, work.d_ngrid.begin());
    amrex::Gpu::copy(amrex::Gpu::hostToDevice,
                     work.h_nslice.begin(), work.h_nslice.begin() + nfield, work.d_nslice.begin());
    amrex::Gpu::copy(amrex::Gpu::hostToDevice,
                     work.h_gridmax.begin(), work.h_gridmax.begin() + nfield, work.d_gridmax.begin());
    amrex::Gpu::copy(amrex::Gpu::hostToDevice,
                     work.h_dgrid.begin(), work.h_dgrid.begin() + nfield, work.d_dgrid.begin());
    amrex::Gpu::copy(amrex::Gpu::hostToDevice,
                     work.h_rtmp.begin(), work.h_rtmp.begin() + nfield, work.d_rtmp.begin());
    amrex::Gpu::copy(amrex::Gpu::hostToDevice,
                     work.h_rharm.begin(), work.h_rharm.begin() + nfield, work.d_rharm.begin());
    amrex::Gpu::copy(amrex::Gpu::hostToDevice,
                     work.h_field_re.begin(), work.h_field_re.begin() + nfield, work.d_field_re.begin());
    amrex::Gpu::copy(amrex::Gpu::hostToDevice,
                     work.h_field_im.begin(), work.h_field_im.begin() + nfield, work.d_field_im.begin());
  }
