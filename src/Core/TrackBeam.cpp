...
AMREX_FORCE_INLINE void advance_plane(
    QuadMode mode,
    double delz,
    double qf,
    double& x,
    ...
    if (mode == QuadMode::Focusing) {
        double foc = std::sqrt(qf / gammaz);
        double omg = foc * delz;
        double a1 = std::cos(omg);
        double a2 = std::sin(omg) / foc;
    ...
    double foc = std::sqrt(-qf / gammaz);
    double omg = foc * delz;
    double a1 = std::cosh(omg);
    double a2 = std::sinh(omg) / foc;
    double a3 = a2 * foc * foc;
    double xtmp = x - dx;
    x  = a1 * xtmp + a2 * px / gammaz + dx;
    px = a3 * xtmp * gammaz + a1 * px;
...
void track_particles_gpu(
    ...
    const long n = static_cast<long>(p->total_particles);

    double* gamma = p->gamma.data();
    double* x     = p->x.data();
    double* y     = p->y.data();
    double* px    = p->px.data();
    double* py    = p->py.data();
    ...
    amrex::ParallelFor(n, [=] AMREX_GPU_DEVICE (long i) noexcept {
        const double gammaz =
            std::sqrt(gamma[i] * gamma[i] - 1.0 - aw * aw - px[i] * px[i] - py[i] * py[i]);

        advance_plane(xmode, delz, qx, x[i], px[i], gammaz, xoff);
        advance_plane(ymode, delz, qy, y[i], py[i], gammaz, yoff);
    });
}
#endif
...
#ifdef GENESIS_USE_AMREX
  QuadMode xmode,ymode;
  if (qx==0){  
    xmode = QuadMode::Drift;
  }else{
    xoff=xoff/qx;
    if (qx>0){
      xmode = QuadMode::Focusing;
    } else {
      xmode = QuadMode::Defocusing;
    }
  }

  if (qy==0){ 
    ymode = QuadMode::Drift;
  }else{
    yoff=yoff/qy;
    if (qy>0){
      ymode = QuadMode::Focusing;
    } else {
      ymode = QuadMode::Defocusing;
    }
  }

  track_particles_gpu(beam->beamSoA,delz,aw,qx,qy,xoff,yoff,xmode,ymode);
  return;
#endif
...
#ifdef GENESIS_USE_AMREX
  Genesis4BeamSoA *p=beam->beamSoA;
  const long n=static_cast<long>(p->total_particles);
...
{ 
  ...
  */
  ...
  const double m23=m[2][3];
  const double m32=m[3][2];
