...
#ifdef GENESIS_USE_AMREX
void Field::init_fieldSoA()
{
    const int nslice = static_cast<int>(field.size());
    const int cells_per_slice = ngrid * ngrid;
    const int total_cells = nslice * cells_per_slice;

    fieldSoA->nslice = nslice;
    fieldSoA->ngrid = ngrid;
    fieldSoA->total_cells = total_cells;

    fieldSoA->field_re.resize(total_cells);
    fieldSoA->field_im.resize(total_cells);

    fieldSoA->initialized = true;
}

void Field::pack_field_to_soa()
{
// cout<<"pack_field_to_soa()"<<endl;
    amrex::Gpu::streamSynchronize();

    if (!fieldSoA->initialized) {
        init_fieldSoA();
    }

    constexpr int g4_field_soa_copy_chunk_cells = 16 * 1024 * 1024;
    const int cells_per_slice = ngrid * ngrid;
    const int chunk_slices = std::max(1, g4_field_soa_copy_chunk_cells / cells_per_slice);
    const int chunk_cells = chunk_slices * cells_per_slice;
    amrex::Gpu::HostVector<double> h_re(chunk_cells);
    amrex::Gpu::HostVector<double> h_im(chunk_cells);

    for (int s = 0; s < fieldSoA->nslice; s += chunk_slices) {
        const int ns = std::min(chunk_slices, fieldSoA->nslice - s);
        const int ncopy = ns * cells_per_slice;
        const int offset = s * cells_per_slice;

        for (int ss = 0; ss < ns; ++ss) {
            const int soff = ss * cells_per_slice;
            const int src = s + ss;
            for (int j = 0; j < cells_per_slice; ++j) {
                h_re[soff + j] = field[src][j].real();
                h_im[soff + j] = field[src][j].imag();
            }
        }

        amrex::Gpu::copyAsync(amrex::Gpu::hostToDevice,
                              h_re.begin(), h_re.begin() + ncopy,
                              fieldSoA->field_re.begin() + offset);
        amrex::Gpu::copyAsync(amrex::Gpu::hostToDevice,
                              h_im.begin(), h_im.begin() + ncopy,
                              fieldSoA->field_im.begin() + offset);
        amrex::Gpu::streamSynchronize();
    }
}

void Field::unpack_soa_to_field()
{
// cout<<"unpack_soa_to_field()"<<endl;
    if (!fieldSoA->initialized) {
        return;
    }
    ...
    for (int s = 0; s < fieldSoA->nslice; s += chunk_slices) {
        ...
        amrex::Gpu::copyAsync(amrex::Gpu::deviceToHost,
                              fieldSoA->field_re.begin() + offset,
                              fieldSoA->field_re.begin() + offset + ncopy,
                              h_re.begin());
        amrex::Gpu::copyAsync(amrex::Gpu::deviceToHost,
                              fieldSoA->field_im.begin() + offset,
                              fieldSoA->field_im.begin() + offset + ncopy,
                              h_im.begin());
        amrex::Gpu::streamSynchronize();
        ...
        for (int ss = 0; ss < ns; ++ss) {
            ...
            const int dst = s + ss;
            for (int j = 0; j < cells_per_slice; ++j) {
                field[dst][j] = complex<double>(h_re[soff + j],
                                                h_im[soff + j]);
            }
        }
    }
