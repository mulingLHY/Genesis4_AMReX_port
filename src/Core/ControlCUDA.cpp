...
void ControlCUDA::releaseWorkBuffers()
{
  if (deviceWork_ != nullptr) {
    amrex::The_Arena()->free(deviceWork_);
    deviceWork_ = nullptr;
    deviceWorkCapacity_ = 0;
  }
  if (pinnedWork_ != nullptr) {
    amrex::The_Pinned_Arena()->free(pinnedWork_);
    pinnedWork_ = nullptr;
    pinnedWorkCapacity_ = 0;
  }
}
...
void ControlCUDA::ensureWorkBuffers(long long needed)
{
  if (deviceWorkCapacity_ < needed) {
    ...
    deviceWork_ = static_cast<double*>(amrex::The_Arena()->alloc(sizeof(double) * static_cast<std::size_t>(needed)));
  ...
  if (pinnedWorkCapacity_ < needed) {
    ...
    pinnedWork_ = static_cast<double*>(amrex::The_Pinned_Arena()->alloc(sizeof(double) * static_cast<std::size_t>(needed)));
