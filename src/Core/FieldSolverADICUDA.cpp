...
struct SourceWorkspace {
  amrex::Gpu::DeviceVector<int>& contrib_key;
  amrex::Gpu::DeviceVector<int>& reduce_key;
  amrex::Gpu::DeviceVector<double>& contrib_re;
  amrex::Gpu::DeviceVector<double>& contrib_im;
  amrex::Gpu::DeviceVector<double>& reduce_re;
  amrex::Gpu::DeviceVector<double>& reduce_im;
  int& capacity;
};
...
void adi_compute_slice_scl(int nslice,
                           ...
                           double* slice_scl)
{
  amrex::ParallelFor(nslice,
    [=] AMREX_GPU_DEVICE (int is) noexcept {
      double scl = 0.0;
                         ...
                         AdiFieldView field)
{
  ...
  adi_solve_x_pcr_kernel<<<nlines, block, shmem, amrex::Gpu::gpuStream()>>>(ngrid,
                                                                            pcr,
                                                                            field);
                         ...
                         AdiFieldView field)
{
  ...
  const int block = g4_next_power_of_two(ngrid);
  const std::size_t shmem = 2 * static_cast<std::size_t>(block) * sizeof(G4ComplexPair);
  adi_solve_y_pcr_kernel<<<nlines, block, shmem, amrex::Gpu::gpuStream()>>>(ngrid,
                                                                            pcr,
                                                                            field);
  return cudaGetLastError() == cudaSuccess;
}
...
void FieldSolverADICUDA::advance(double delz, Field *field, Beam *beam, Undulator *und) {
  ...
  if (field == nullptr || beam == nullptr || und == nullptr) {
    amrex::Abort("FieldSolverADICUDA::advance received a null field/beam/undulator pointer");
  }
  ...
  if (bsoa == nullptr || !bsoa->initialized ||
    ...
    amrex::Abort("FieldSolverADICUDA::advance requires initialized BeamSoA/FieldSoA and matching ngrid");
  }
  ...
  if (nslice <= 0 || plane <= 0 || total_cells <= 0 ||
    ...
    amrex::Abort("FieldSolverADICUDA::advance found inconsistent SoA dimensions");
  }
  ...
  if (static_cast<int>(d_c_re.size()) < ngrid_i ||
    ...
    amrex::Abort("FieldSolverADICUDA::advance was called before GPU ADI coefficients/PCR factors were initialized");
  }
...
void FieldSolverADICUDA::init(double delz, double dgrid, double xks, unsigned int ngrid_in) {
  ...
  for (int i = 0; i < static_cast<int>(ngrid); ++i) {
    d_c_re[i] = c_host[i].real();
    d_c_im[i] = c_host[i].imag();
    d_cbet_re[i] = cbet_host[i].real();
    d_cbet_im[i] = cbet_host[i].imag();
    d_cwet_re[i] = cwet_host[i].real();
    d_cwet_im[i] = cwet_host[i].imag();
  }
