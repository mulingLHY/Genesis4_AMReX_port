                                                    ...
                                                    const char* where)
{
  if (result != CUFFT_SUCCESS) {
    amrex::Abort(std::string("FieldSolverFFTCUDA cuFFT failure in ") + where +
                                                   ...
                                                   const char* where)
{
  if (result != cudaSuccess) {
    amrex::Abort(std::string("FieldSolverFFTCUDA CUDA failure in ") + where +
                                               ...
                                               cufftDoubleComplex* source_fft)
{
  ...
  amrex::ParallelFor(total_particles,
    [=] AMREX_GPU_DEVICE (int ip) noexcept {
      ...
      amrex::Gpu::Atomic::Add(&source_fft[base].x, cpart_re * w00);
      amrex::Gpu::Atomic::Add(&source_fft[base].y, cpart_im * w00);

      amrex::Gpu::Atomic::Add(&source_fft[base + 1].x, cpart_re * w10);
      amrex::Gpu::Atomic::Add(&source_fft[base + 1].y, cpart_im * w10);

      amrex::Gpu::Atomic::Add(&source_fft[base + ngrid].x, cpart_re * w01);
      amrex::Gpu::Atomic::Add(&source_fft[base + ngrid].y, cpart_im * w01);

      amrex::Gpu::Atomic::Add(&source_fft[base + ngrid + 1].x, cpart_re * w11);
      amrex::Gpu::Atomic::Add(&source_fft[base + ngrid + 1].y, cpart_im * w11);
    });
}
...
void FieldSolverFFTCUDA::ensure_gpu_plan(int nslice)
{
  ...
  genesis4_fieldsolverfftcuda_abort_if_cufft_failed(
      cufftSetStream(gpu_forward_plan, amrex::Gpu::gpuStream()),
      "cufftSetStream forward");
  genesis4_fieldsolverfftcuda_abort_if_cufft_failed(
      cufftSetStream(gpu_inverse_plan, amrex::Gpu::gpuStream()),
...
void FieldSolverFFTCUDA::init(double delz,double dgrid, double xks, unsigned int ngrid_in)
{
  ...
  const int ngrid_i = static_cast<int>(ngrid);
  const int plane = ngrid_i * ngrid_i;
  d_k2_im.resize(plane);
  d_sigmoid.resize(plane);
  ...
  for (int iy=0;iy<ngrid_i;iy++) {
    ...
    for (int ix=0;ix<ngrid_i;ix++) {
      ...
      d_k2_im[ii] = -(dx*dx+dy*dy)*dk*dk/2./xks;
      if (filter_shape_valid) {
        double r = (sqrt(x * x + y * y) - 1) / sig;
        d_sigmoid[ii] = 1. / (1 + exp(r));
      } else {
        d_sigmoid[ii] = 1.0;
      }
    }
...
void FieldSolverFFTCUDA::advance(double delz, Field *field, Beam *beam, Undulator *und)
{
  if (field == nullptr || beam == nullptr || und == nullptr) {
    amrex::Abort("FieldSolverFFTCUDA::advance_gpu received a null field/beam/undulator pointer");
  }
  ...
  if (bsoa == nullptr || !bsoa->initialized ||
    ...
    amrex::Abort("FieldSolverFFTCUDA::advance_gpu requires initialized BeamSoA/FieldSoA and matching ngrid");
  }
  ...
  if (nslice <= 0 || plane <= 0 || total_cells <= 0 ||
    ...
    amrex::Abort("FieldSolverFFTCUDA::advance_gpu found inconsistent SoA dimensions");
  }
