...
#endif
{// To ensure local variables are delete before AMReX::Finalize() is called
    //-------------------------------------------------------
...
#ifdef GENESIS_USE_AMREX
    if (use_cuda) {
        int dev = amrex::Gpu::Device::deviceId();
        cudaDeviceProp prop;
...
#ifdef GENESIS_USE_AMREX
if (use_cuda) { amrex::Gpu::streamSynchronize(); }
#endif
t0 = MPI_Wtime();
