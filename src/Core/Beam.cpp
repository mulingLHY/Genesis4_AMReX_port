...
void Beam::init_beamSoA()
{
    ...
    amrex::Gpu::HostVector<int> h_slice_id(total_particles);
    ...
    if (total_particles > 0) {
        amrex::Gpu::copyAsync(amrex::Gpu::hostToDevice,
                              h_slice_id.begin(), h_slice_id.end(),
                              beamSoA->slice_id.begin());
    }
    amrex::Gpu::copyAsync(amrex::Gpu::hostToDevice,
                          h_slice_offsets.begin(), h_slice_offsets.end(),
...
// cout<<"pack_beam_to_soa"<<endl;
    ...
    amrex::Gpu::HostVector<double> h_x(buffer_particles);
    amrex::Gpu::HostVector<double> h_y(buffer_particles);
    amrex::Gpu::HostVector<double> h_px(buffer_particles);
    amrex::Gpu::HostVector<double> h_py(buffer_particles);
    amrex::Gpu::HostVector<double> h_gamma(buffer_particles);
    amrex::Gpu::HostVector<double> h_theta(buffer_particles);
    ...
    auto flush_chunk = [&]() {
        ...
        amrex::Gpu::copyAsync(amrex::Gpu::hostToDevice,
                              h_x.begin(), h_x.begin() + chunk_count,
                              beamSoA->x.begin() + chunk_offset);
        amrex::Gpu::copyAsync(amrex::Gpu::hostToDevice,
                              h_y.begin(), h_y.begin() + chunk_count,
                              beamSoA->y.begin() + chunk_offset);
        amrex::Gpu::copyAsync(amrex::Gpu::hostToDevice,
                              h_px.begin(), h_px.begin() + chunk_count,
                              beamSoA->px.begin() + chunk_offset);
        amrex::Gpu::copyAsync(amrex::Gpu::hostToDevice,
                              h_py.begin(), h_py.begin() + chunk_count,
                              beamSoA->py.begin() + chunk_offset);
        amrex::Gpu::copyAsync(amrex::Gpu::hostToDevice,
                              h_gamma.begin(), h_gamma.begin() + chunk_count,
                              beamSoA->gamma.begin() + chunk_offset);
        amrex::Gpu::copyAsync(amrex::Gpu::hostToDevice,
                              h_theta.begin(), h_theta.begin() + chunk_count,
...
// cout<<"unpack_soa_to_beam"<<endl;
    ...
    amrex::Gpu::HostVector<double> h_gamma(buffer_particles);
    amrex::Gpu::HostVector<double> h_theta(buffer_particles);
    amrex::Gpu::HostVector<int> h_slice_offsets(beamSoA->nslice + 1);
