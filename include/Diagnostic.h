...
#ifdef GENESIS_USE_AMREX
 bool calc_builtin_amrex(Beam *, std::vector<Field *> *, double);
 bool calc_builtin_beam_amrex(Beam *);
 bool calc_builtin_field_amrex(Field *, int groupID);
 FilterDiagnostics filter_;
 std::unique_ptr<DiagnosticAMReXScratch> amrex_scratch_;
#endif
 bool time,scan;
